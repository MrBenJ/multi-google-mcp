...
> - `multi-google-mcp` — the server itself (Codex will start it automatically)
...
1. Tell them: *"That's fine. The server will be wired into Codex in the next step, but it won't do anything useful until you add at least one account. When you're ready, just run `multi-google-mcp-auth add <label>` from any terminal."*
...
The install is done on disk. Now we confirm Codex picks up the change.
...
**Tools don't appear / Codex doesn't call any `gmail_*` tool:**
...
3. Verify the config one more time:
   ...
   grep -A2 '^\[mcp_servers\.multi-google\]' "$HOME/.codex/config.toml"
   ```
   Should print the section followed by the command line.
4. Check Codex's session log (`~/.codex/log/` or run `codex --debug`) for an error starting the `multi-google` server.
